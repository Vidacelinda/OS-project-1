...
#include <iostream>
// NOTE : my mac is having issues runing #include<bits/stdc++.h> but it works on pc running windows
// C++
#include <bits/stdc++.h>
//
//  SJF C++.cpp
//
//
//  Created by Carlo Leiva on 10/8/22.
//

using namespace std;

typedef pair<int, int> pi;
void sjf(vector<vector<int>> processes,
...
void printQueue(priority_queue<pi, vector<pi>, greater<pi>> q,
                vector<vector<int>> processes, vector<int> processBurstIndex);
void printContextSwitchData(
    vector<vector<int>> processes, int process_index,
    priority_queue<pi, vector<pi>, greater<pi>> readyQueue,
...
void sjf(vector<vector<int>> processes,priority_queue<pi, vector<pi>, greater<pi>> readyQueue,vector<int> processBurstIndex) {
  ...
  while (!readyQueue.empty()) {
    ...
    // takes value of first waiting time and then resets flag so the if
    // condition never executes again
    if (responseTime[process_index] == -1) {
      responseTime[process_index] = time - arrivalTime[process_index];
    }
    printContextSwitchData(processes, process_index, readyQueue,
    ...
    for (int i = 0; i < processes.size(); i++) {
      if (ioreturn[i].first <= time) {
        readyQueue.push(make_pair(
            processes[ioreturn[i].second]
                     [processBurstIndex[ioreturn[i].second]],
            ioreturn[i].second)); // pushing process id into readyQueue
 ...
 cout<<"results"<<endl;
  ...
  cout << "Total time required to complete all the 8 processes: " << time
       << endl;
  ...
  cout << "Average Waiting Time: " << (float)twt / processes.size() << endl
       << endl;
  cout << "_________________________________________________________ " << endl<<endl;
